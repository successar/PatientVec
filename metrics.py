...
def calc_metrics_multitask(target, predictions) :
    ...
    for t in range(n_tasks) :
        idxs = np.where(target[:, t, 1] == 1)[0]
        task_target = target[idxs, t, 0]
        task_prediction = predictions[idxs, t]
        task_rep = calc_metrics_classification(task_target, task_prediction)
        for k in list(task_rep.keys()) :
            task_rep['task_' + str(t) + '/' + k] = task_rep[k]
            del task_rep[k]

        tasks_reps.update(task_rep)
    
    macro_roc_auc = np.mean([v for k, v in tasks_reps.items() if 'roc_auc' in k])
    macro_pr_auc = np.mean([v for k, v in tasks_reps.items() if 'pr_auc' in k])
    
    tasks_reps['macro_roc_auc'] = macro_roc_auc
    tasks_reps['macro_pr_auc'] = macro_pr_auc
...
metrics_map = {
    ...
    'multitask' : calc_metrics_multitask
}
